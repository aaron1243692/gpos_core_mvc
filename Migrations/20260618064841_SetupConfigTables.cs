...
{
    /// <inheritdoc />
    public partial class SetupConfigTables : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(
                """
                SET @legacy_employee_table_exists = (
                    SELECT COUNT(*)
                ...
                SET @setup_employee_table_exists = (
                    SELECT COUNT(*)
                    FROM INFORMATION_SCHEMA.COLUMNS
                    WHERE TABLE_SCHEMA = DATABASE()
                      AND TABLE_NAME = 'employees'
                ...
                SET @employee_accounts_table_exists = (
                    SELECT COUNT(*)
                ...
                SET @statement = IF(
                    @legacy_employee_table_exists > 0
                    AND @setup_employee_table_exists = 0
                    AND @employee_accounts_table_exists = 0,
                    'RENAME TABLE `employees` TO `employee_accounts`',
                    'SELECT 1'
                );

                PREPARE rename_legacy_employees FROM @statement;
                EXECUTE rename_legacy_employees;
                DEALLOCATE PREPARE rename_legacy_employees;
                ...
                SET @employee_account_username_index_exists = (
                    SELECT COUNT(*)
                    FROM INFORMATION_SCHEMA.STATISTICS
                    WHERE TABLE_SCHEMA = DATABASE()
                      AND TABLE_NAME = 'employee_accounts'
                      AND INDEX_NAME = 'IX_employee_accounts_username'
                );

                SET @statement = IF(
                    @employee_account_username_index_exists = 0,
                    'CREATE UNIQUE INDEX `IX_employee_accounts_username` ON `employee_accounts` (`username`)',
                    'SELECT 1'
                );
